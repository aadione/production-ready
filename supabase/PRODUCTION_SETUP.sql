-- =============================================================================
-- Jamshedpurwala — complete production database setup
--
-- Run this ONCE in the Supabase SQL editor of your project:
--   https://supabase.com/dashboard/project/prulydvoiijjojojzmrg/sql/new
--
-- It is fully idempotent: safe to run on an empty project AND safe to re-run on
-- a project that already has some of these objects. It never drops a table and
-- never deletes customer data.
-- =============================================================================
...
-- customers must never be able to write their own PIN hash
...
-- read-only for customers: no INSERT/UPDATE/DELETE grant at all, so prices and
-- stock can never be changed from the browser.
...
-- one canonical set of status values, matching the frontend labels
...
-- Read-only for customers. Orders are created ONLY by place_order() below, so
-- the browser can never insert a fake total or edit an existing order.
...
CREATE TRIGGER addresses_updated BEFORE UPDATE ON public.addresses
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
DROP TRIGGER IF EXISTS carts_updated ON public.carts;
...
BEGIN
  ...
  ON CONFLICT (id) DO NOTHING;   -- never creates a duplicate profile
  RETURN NEW;
END;
$$;
...
-- 8. ONE DEFAULT ADDRESS PER USER --------------------------------------------
...
-- 10. ATOMIC, SERVER-PRICED ORDER PLACEMENT ----------------------------------
-- The only way an order can be created. Everything below runs in ONE
-- transaction: any failure rolls back the order, the items, the stock decrement
-- and the cart cleanup together.
...
BEGIN
  ...
  -- Address must belong to the caller. A tampered address id fails here.
  ...
  -- Serialise concurrent placements for this customer. A double-click sends the
  -- second request into this lock, so the two never interleave.
  PERFORM pg_advisory_xact_lock(hashtext('place_order:' || v_user::text));

  INSERT INTO public.orders (
    user_id, shipping_address, subtotal, discount, delivery_fee, total_amount,
    payment_method, payment_status, order_status)
  VALUES (v_user, jsonb_build_object(
            'full_name', v_addr.full_name, 'phone', v_addr.phone, 'line1', v_addr.line1,
            'line2', v_addr.line2, 'city', v_addr.city, 'state', v_addr.state,
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    END IF;
    IF v_pid = ANY(v_ids) THEN            -- same product listed twice
      RAISE EXCEPTION 'QUANTITY_INVALID';
    END IF;

    -- Lock the product row, then read the REAL price/MRP/stock from the table.
    SELECT * INTO v_prod FROM public.products
     WHERE id = v_pid AND is_active FOR UPDATE;
    IF NOT FOUND THEN
    ...
    INSERT INTO public.order_items (
      order_id, user_id, product_id, product_name, product_brand,
      shop_name, price, mrp, quantity, subtotal)
    VALUES (v_order_id, v_user, v_prod.id, v_prod.name, v_prod.brand,
            COALESCE(v_prod.store_name, v_prod.brand),
...
-- 11. PRODUCT CATALOGUE SEED --------------------------------------------------
-- Prices/stock are only inserted when the product does not exist yet, so
-- re-running this script never overwrites stock you have since changed.
